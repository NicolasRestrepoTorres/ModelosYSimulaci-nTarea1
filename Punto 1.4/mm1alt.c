...
int   next_event_type, num_custs_delayed, num_events, num_in_q, server_status, machine_time, tolerance;
float area_num_in_q, area_server_status,
      sim_time, time_arrival[Q_LIMIT + 1], time_end, time_last_event, 
      time_next_event[5], total_of_delays;
FILE  *infile, *outfile;
...
void initialize(void)  /* Initialization function. */
{
    ...
    time_next_event[1] = sim_time + 15 + rand() % 6;
    time_next_event[2] = 1.0e+30;
    time_next_event[3] = sim_time + 3600;
...
void arrive(void)  /* Arrival event function. */
{
    ...
    if (server_status == BUSY) {
        ...
        time_next_event[2] = sim_time + time_mac();
    }
}
...
float time_mac(void){
	return machine_time + rand() % tolerance;
}
...
void report(void)  /* Report generator function. */
{
    ...
    fprintf(outfile, "Number of pieces completed%7d",
            num_custs_delayed);
}
